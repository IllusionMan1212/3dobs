...
impl Renderer {
  ...
  ) -> Self
  {
    ...
    unsafe {
      ...
      let vert_sources = [glsl_version.as_ptr() as *const gl::GLchar,
                          vert_source.as_ptr() as *const gl::GLchar];
      let vert_sources_len = [glsl_version.len() as gl::GLint - 1,
                              vert_source.len() as gl::GLint - 1];
      let frag_sources = [glsl_version.as_ptr() as *const gl::GLchar,
                          frag_source.as_ptr() as *const gl::GLchar];
      let frag_sources_len = [glsl_version.len() as gl::GLint - 1,
                              frag_source.len() as gl::GLint - 1];
  ...
  pub fn render(
    ...
    unsafe {
      ...
      let last_polygon_mode = return_param(|x: &mut [gl::GLint; 2]| gl::GetIntegerv(gl::POLYGON_MODE, x.as_mut_ptr()));
      let last_viewport = return_param(|x: &mut [gl::GLint; 4]| gl::GetIntegerv(gl::VIEWPORT, x.as_mut_ptr()));
      let last_scissor_box = return_param(|x: &mut [gl::GLint; 4]| gl::GetIntegerv(gl::SCISSOR_BOX, x.as_mut_ptr()));
        ...
        for cmd in draw_list.commands() {
          match cmd {
            DrawCmd::Elements {
              ...
              gl::Scissor((x * scale_w) as gl::GLint,
                         (fb_height - w * scale_h) as gl::GLint,
                         ((z - x) * scale_w) as gl::GLint,
                         ((w - y) * scale_h) as gl::GLint);
