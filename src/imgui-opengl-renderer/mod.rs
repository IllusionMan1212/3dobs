...
impl Renderer {
    pub fn new(imgui: &mut Context) -> Self {
      ...
      \0";
            ...
            let vert_sources = [
                glsl_version.as_ptr() as *const gl::GLchar,
                vert_source.as_ptr() as *const gl::GLchar,
            ];
            let vert_sources_len = [
                glsl_version.len() as gl::GLint - 1,
    ...
    pub fn render(&self, ctx: &mut Context) {
        ...
        unsafe {
            ...
            let last_polygon_mode = return_param(|x: &mut [gl::GLint; 2]| {
                gl::GetIntegerv(gl::POLYGON_MODE, x.as_mut_ptr())
            });
            let last_viewport = return_param(|x: &mut [gl::GLint; 4]| {
                gl::GetIntegerv(gl::VIEWPORT, x.as_mut_ptr())
            });
            let last_scissor_box = return_param(|x: &mut [gl::GLint; 4]| {
